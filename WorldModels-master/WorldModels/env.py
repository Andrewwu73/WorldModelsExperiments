...
class CarRacingMDNRNN(CarRacingWrapper):
  ...
  def reset(self):
    ...
    if self.with_obs:
        ...
        return [rnn_output(self.rnn_states, self.encode_obs(z_state), 4), z_state]
    else:
        z_state = super(CarRacingMDNRNN, self).reset() # calls step
        self.N_tiles = len(self.track)
        #print(rnn_output(self.rnn_states, self.encode_obs(z_state), 4))
        return rnn_output(self.rnn_states, self.encode_obs(z_state), 4)
  def _step(self, action):
